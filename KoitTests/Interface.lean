...
#guard has v6_8 "program p : syscall {\n  hold nothing { let x = 1 }\n  return 0\n}\n"
  "not a resource acquisition on kernel v6.8: the resources the interface declares are `lock(p)`, `rcu`, `rb.reserve<T>()`"
#guard has v6_8 "program p : syscall {\n  let r = get_prandom_u32()\n  return 0\n}\n"
  "is the kernel's helper bpf_get_prandom_u32, which koit has no declaration for yet"
#guard has v6_8 "program p : xdp default { pass } {\n  hold rcu { let x = bpf_rcu_read_lock() }\n  pass\n}\n"
  "is a kfunc of the kernel, which koit has no declaration for yet"
#guard has v6_8 "program p : syscall {\n  let r = redirect(1)?\n  return 0\n}\n"
